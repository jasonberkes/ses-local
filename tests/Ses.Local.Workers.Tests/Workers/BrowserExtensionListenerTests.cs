...
public sealed class BrowserExtensionListenerTests
{
    ...
    [Fact]
    public void ExtensionPayload_WithoutSource_DefaultsToClaudeChat()
    {
        var json = """
            {
              "conversations": [{
        ...
        var payload = JsonSerializer.Deserialize(json, ExtensionPayloadJsonContext.Default.ExtensionSyncPayload);
        Assert.NotNull(payload);
        Assert.Null(payload.Conversations[0].Source); // null source → defaults to ClaudeChat in ResolveSource
    }
