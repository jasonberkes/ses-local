...
/// </summary>
public sealed class ImportWizardViewModel : INotifyPropertyChanged, IDisposable
{
    /// <summary>
    ...
    /// <summary>Production constructor — wires to <see cref="DaemonAuthProxy"/> methods.</summary>
    ...
    /// <summary>Testable constructor — accepts pure functions so tests can inject fakes.</summary>
    ...
    /// <summary>Step 1 → 2: select source and pre-populate file path (for re-import).</summary>
    public void SelectSourceWithFile(ImportSource source, string filePath)
    {
        SelectedSource = source;
        FilePath       = filePath;
    ...
    /// <summary>Step 4 → 1: reset wizard to choose another source.</summary>
    public void Reset()
    {
        _pollCts?.Cancel();
        _pollCts?.Dispose();
    ...
    private async Task PollProgressAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            catch { /* Daemon temporarily unreachable — keep polling */ }
        }
    }
